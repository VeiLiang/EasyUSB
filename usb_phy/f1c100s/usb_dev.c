...
static unsigned int fifo_offset = 0;

void usb_config_ep_in(int epidx,int maxpack,int type)
{
	u32 old_ep_idx  = 0;
	if(epidx && (fifo_offset + maxpack <= 1024*4))
	{
		...
		USBC_ConfigFifo(USBC_EP_TYPE_TX,0,maxpack*2,fifo_offset);
		fifo_offset += maxpack*2;
...
void usb_config_ep_out(int epidx,int maxpack,int type)
{
	...
	if(epidx && (fifo_offset + maxpack <= 1024*4))
	{
		...
		USBC_ConfigFifo(USBC_EP_TYPE_RX,0,maxpack*2,fifo_offset);
		fifo_offset += maxpack*2;
